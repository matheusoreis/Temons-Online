    ...
    Select Case MenuState
        ...
        Case MenuStateEnum.StateNormal
            ...
            For i = ButtonEnum.Menu_Website To ButtonEnum.Menu_ChangePass
                If CanShowButton(i) Then
                    RenderTexture Tex_Gui(MenuUi_Texture), Button(i).X, Button(i).Y, Button(i).StartX(Button(i).State), Button(i).StartY(Button(i).State), Button(i).Width, Button(i).Height, Button(i).Width, Button(i).Height
                End If
            Next
                ...
                If GuiVisibleCount > 0 Then
                    For i = 1 To GuiVisibleCount
                        If CanShowGui(GuiZOrder(i)) Then
                            '//The first one will be rendered first
                            Select Case GuiZOrder(i)
                                Case GuiEnum.GUI_LOGIN:             DrawLogin
                                Case GuiEnum.GUI_REGISTER:          DrawRegister
                                Case GuiEnum.GUI_CHARACTERSELECT:   DrawCharacterSelect
                                Case GuiEnum.GUI_CHARACTERCREATE:   DrawCharacterCreate
                            End Select
                        End If
                    Next
                End If
...
Dim i As Long
    ...
    'RenderTexture Tex_Surface(gSurfaceEnum.Background), 0, 0, 0, 0, Screen_Width, Screen_Height, Width, Height
    RenderTexture Tex_System(gSystemEnum.UserInterface), 0, Screen_Height - 40, 0, 8, Screen_Width - 80, 40, 1, 1, D3DColorARGB(90, 0, 0, 0)
    RenderText Font_Default, "Pokemon is Copyright © 1995 - 2016  Nintendo / Creatures Inc. / GAME FREAK Inc.", 10, Screen_Height - 35, White
    RenderText Font_Default, "PokeNew is not affiliated with Nintendo, Creatures Inc. and GAME FREAK Inc.", 10, Screen_Height - 20, White
    ...
    If CursorX >= textX And CursorX <= textX + 80 And CursorY >= textY And CursorY <= textY + 40 Then
        IsHovering = True
        MouseIcon = 1 '//Select
        RenderTexture Tex_System(gSystemEnum.UserInterface), textX, textY, 0, 8, 80, 40, 1, 1, D3DColorARGB(140, 0, 0, 0)
    Else
        RenderTexture Tex_System(gSystemEnum.UserInterface), textX, textY, 0, 8, 80, 40, 1, 1, D3DColorARGB(100, 0, 0, 0)
    End If
    ...
    Else
        textCredit = "Credits"
    End If
    RenderText Font_Default, textCredit, textX + (40 - (GetTextWidth(Font_Default, textCredit) / 2)), textY + 12, White

    DrawCredit
End Sub

Private Sub DrawCredit()
Dim i As Long
...
Dim X As Long
    ...
    With GUI(GuiEnum.GUI_LOGIN)
        ...
        Select Case CurTextbox
            Case 1 ' User
                RenderText Font_Default, UpdateChatText(Font_Default, User, 180) & TextLine, .X + 88, .Y + 46, White
                RenderText Font_Default, UpdateChatText(Font_Default, CensorWord(Pass), 180), .X + 88, .Y + 79, White
            Case 2 ' Pass
                RenderText Font_Default, UpdateChatText(Font_Default, User, 180), .X + 88, .Y + 46, White
                RenderText Font_Default, UpdateChatText(Font_Default, CensorWord(Pass), 180) & TextLine, .X + 88, .Y + 79, White
        End Select
        
        '//Misc
        '//Checkbox for saving account details
        If CursorX >= .X + 65 And CursorX <= .X + 65 + 17 And CursorY >= .Y + 140 And CursorY <= .Y + 140 + 17 And GuiZOrder(GuiVisibleCount) = GuiEnum.GUI_LOGIN Then
            RenderTexture Tex_Gui(.Pic), .X + 65, .Y + 140, 319, 125 + 17, 17, 17, 17, 17
        Else
            RenderTexture Tex_Gui(.Pic), .X + 65, .Y + 140, 319, 125, 17, 17, 17, 17
        End If
        
        RenderText Font_Default, "", .X + 23 + 110, .Y + 112, White, , 220
        
        If ShowServerList Then
            'If CursorX >= 10 And CursorX <= 10 + (ServerMaxWidth + 6) And CursorY >= Screen_Height - 40 - 30 And CursorY <= (Screen_Height - 40 - 30) + 20 Then
            '    RenderTexture Tex_System(gSystemEnum.UserInterface), 10, Screen_Height - 40 - 30, 0, 8, ServerMaxWidth + 6, 20, 1, 1, D3DColorARGB(100, 50, 50, 50)
            'Else
            '    RenderTexture Tex_System(gSystemEnum.UserInterface), 10, Screen_Height - 40 - 30, 0, 8, ServerMaxWidth + 6, 20, 1, 1, D3DColorARGB(100, 0, 0, 0)
            'End If
            If CurServerList > 0 Then
                RenderText Font_Default, ServerName(CurServerList), .X + 23 + 110, .Y + 112, White
            End If
                
            If ServerList Then
                For X = 1 To MAX_SERVER_LIST
                    If CursorX >= .X + 20 + 110 And CursorX <= .X + 20 + 110 + 140 And CursorY >= .Y + 112 + ((20 * MAX_SERVER_LIST) + ((X - 2) * 20)) And CursorY <= .Y + 112 + ((20 * MAX_SERVER_LIST) + ((X - 2) * 20)) + 20 Then
                        RenderTexture Tex_System(gSystemEnum.UserInterface), .X + 20 + 110, .Y + 112 + ((20 * MAX_SERVER_LIST) + ((X - 2) * 20)), 0, 8, 140, 20, 1, 1, D3DColorARGB(150, 50, 50, 50)
                    Else
                        RenderTexture Tex_System(gSystemEnum.UserInterface), .X + 20 + 110, .Y + 112 + ((20 * MAX_SERVER_LIST) + ((X - 2) * 20)), 0, 8, 140, 20, 1, 1, D3DColorARGB(150, 0, 0, 0)
                    End If
                    RenderText Font_Default, ServerName(X), .X + 23 + 110 + 2, .Y + 112 + ((20 * MAX_SERVER_LIST) + ((X - 2) * 20)) + 2, White
                Next
            End If
        End If
        
        If GameSetting.SavePass = YES Then
            RenderTexture Tex_Gui(.Pic), .X + 65, .Y + 140, 319, 125 + 34, 17, 17, 17, 17
        End If
    End With
End Sub
...
Dim PassColor As Long
    ...
    With GUI(GuiEnum.GUI_REGISTER)
        ...
        '//Checking if password confirmation is the same with password
        If Len(Pass2) > 0 Then
            If Pass <> Pass2 Then
                PassColor = BrightRed
            Else
        ...
        Select Case CurTextbox
            Case 1 ' User
                ...
                If ShowPass = YES Then
                    RenderText Font_Default, UpdateChatText(Font_Default, Pass, 180), .X + 88, .Y + 79, PassColor
                    RenderText Font_Default, UpdateChatText(Font_Default, Pass2, 180), .X + 88, .Y + 100, PassColor
                Else
                    RenderText Font_Default, UpdateChatText(Font_Default, CensorWord(Pass), 180), .X + 88, .Y + 79, PassColor
                    RenderText Font_Default, UpdateChatText(Font_Default, CensorWord(Pass2), 180), .X + 88, .Y + 100, PassColor
                End If
                RenderText Font_Default, UpdateChatText(Font_Default, Email, 180), .X + 88, .Y + 167, White
            Case 2 ' Pass
                RenderText Font_Default, UpdateChatText(Font_Default, User, 180), .X + 88, .Y + 46, White
                If ShowPass = YES Then
                    RenderText Font_Default, UpdateChatText(Font_Default, Pass, 180) & TextLine, .X + 88, .Y + 79, PassColor
                    RenderText Font_Default, UpdateChatText(Font_Default, Pass2, 180), .X + 88, .Y + 100, PassColor
                Else
                    RenderText Font_Default, UpdateChatText(Font_Default, CensorWord(Pass), 180) & TextLine, .X + 88, .Y + 79, PassColor
                    RenderText Font_Default, UpdateChatText(Font_Default, CensorWord(Pass2), 180), .X + 88, .Y + 100, PassColor
                End If
                RenderText Font_Default, UpdateChatText(Font_Default, Email, 180), .X + 88, .Y + 167, White
            Case 3 ' Retype Pass
                ...
                If ShowPass = YES Then
                    RenderText Font_Default, UpdateChatText(Font_Default, Pass, 180), .X + 88, .Y + 79, PassColor
                    RenderText Font_Default, UpdateChatText(Font_Default, Pass2, 180) & TextLine, .X + 88, .Y + 100, PassColor
                Else
                    RenderText Font_Default, UpdateChatText(Font_Default, CensorWord(Pass), 180), .X + 88, .Y + 79, PassColor
                    RenderText Font_Default, UpdateChatText(Font_Default, CensorWord(Pass2), 180) & TextLine, .X + 88, .Y + 100, PassColor
                End If
                RenderText Font_Default, UpdateChatText(Font_Default, Email, 180), .X + 88, .Y + 167, White
            Case 4 ' Email
                RenderText Font_Default, UpdateChatText(Font_Default, User, 180), .X + 88, .Y + 46, White
                ...
                Else
                    ...
                    RenderText Font_Default, UpdateChatText(Font_Default, CensorWord(Pass2), 180), .X + 88, .Y + 100, PassColor
                End If
                RenderText Font_Default, UpdateChatText(Font_Default, Email, 180) & TextLine, .X + 88, .Y + 167, White
        End Select
        
        '//Preview of Password Strength
        If Len(Pass) > 0 Then
            If Len(Pass) >= 0 And Len(Pass) < ((NAME_LENGTH - 1) / 4) Then
                RenderTexture Tex_System(gSystemEnum.UserInterface), .X + 86, .Y + 125, 0, 8, 46, 4, 1, 1, D3DColorARGB(255, 237, 28, 36)
            ElseIf Len(Pass) >= ((NAME_LENGTH - 1) / 4) And Len(Pass) < ((NAME_LENGTH - 1) / 4) * 2 Then
                RenderTexture Tex_System(gSystemEnum.UserInterface), .X + 86, .Y + 125, 0, 8, 46, 4, 1, 1, D3DColorARGB(255, 198, 153, 0)
                RenderTexture Tex_System(gSystemEnum.UserInterface), .X + 134, .Y + 125, 0, 8, 46, 4, 1, 1, D3DColorARGB(255, 198, 153, 0)
            ElseIf Len(Pass) >= ((NAME_LENGTH - 1) / 4) * 2 And Len(Pass) < ((NAME_LENGTH - 1) / 4) * 3 Then
                RenderTexture Tex_System(gSystemEnum.UserInterface), .X + 86, .Y + 125, 0, 8, 46, 4, 1, 1, D3DColorARGB(255, 158, 203, 22)
                RenderTexture Tex_System(gSystemEnum.UserInterface), .X + 134, .Y + 125, 0, 8, 46, 4, 1, 1, D3DColorARGB(255, 158, 203, 22)
                RenderTexture Tex_System(gSystemEnum.UserInterface), .X + 182, .Y + 125, 0, 8, 46, 4, 1, 1, D3DColorARGB(255, 158, 203, 22)
            ElseIf Len(Pass) >= ((NAME_LENGTH - 1) / 4) * 3 And Len(Pass) < ((NAME_LENGTH - 1) + 5) Then
                RenderTexture Tex_System(gSystemEnum.UserInterface), .X + 86, .Y + 125, 0, 8, 46, 4, 1, 1, D3DColorARGB(255, 0, 162, 232)
                RenderTexture Tex_System(gSystemEnum.UserInterface), .X + 134, .Y + 125, 0, 8, 46, 4, 1, 1, D3DColorARGB(255, 0, 162, 232)
                RenderTexture Tex_System(gSystemEnum.UserInterface), .X + 182, .Y + 125, 0, 8, 46, 4, 1, 1, D3DColorARGB(255, 0, 162, 232)
                RenderTexture Tex_System(gSystemEnum.UserInterface), .X + 230, .Y + 125, 0, 8, 46, 4, 1, 1, D3DColorARGB(255, 0, 162, 232)
            End If
        End If
        
        '//Misc
        '//Checkbox for saving account details
        If CursorX >= .X + 65 And CursorX <= .X + 65 + 17 And CursorY >= .Y + 137 And CursorY <= .Y + 137 + 17 And GuiZOrder(GuiVisibleCount) = GuiEnum.GUI_REGISTER Then
            RenderTexture Tex_Gui(.Pic), .X + 65, .Y + 137, 319, 125 + 17, 17, 17, 17, 17
        Else
            RenderTexture Tex_Gui(.Pic), .X + 65, .Y + 137, 319, 125, 17, 17, 17, 17
        End If
        
        If ShowPass = YES Then
            RenderTexture Tex_Gui(.Pic), .X + 65, .Y + 137, 319, 125 + 34, 17, 17, 17, 17
        End If
    End With
End Sub

Private Sub DrawCharacterSelect()
Dim i As Long
...
Dim Sprite As Long
    ...
    With GUI(GuiEnum.GUI_CHARACTERSELECT)
        ...
        Else
            CharNameText = "Empty"
        End If
        RenderText Font_Default, CharNameText, .X + ((.Width / 2) - (GetTextWidth(Font_Default, CharNameText) / 2)) - 2, .Y + 37, D3DColorARGB(180, 250, 250, 250), False
    
        '//Character Sprite
        If pCharInUsed(CurChar) Then
            Sprite = pCharSprite(CurChar)
            If Sprite > 0 Then
                RenderTexture Tex_Character(Sprite), .X + ((.Width / 2) - (((GetPicWidth(Tex_Character(Sprite)) / 3) * 2) / 2)), .Y + 75, (GetPicWidth(Tex_Character(Sprite)) / 3), 0, (GetPicWidth(Tex_Character(Sprite)) / 3) * 2, (GetPicHeight(Tex_Character(Sprite)) / 4) * 2, GetPicWidth(Tex_Character(Sprite)) / 3, GetPicHeight(Tex_Character(Sprite)) / 4, D3DColorARGB(255, 255, 255, 255)
            End If
        End If
    End With
End Sub

Private Sub DrawCharacterCreate()
Dim i As Long
Dim Sprite As Long
Dim Width As Long, Height As Long
    ...
    With GUI(GuiEnum.GUI_CHARACTERCREATE)
        ...
        For i = ButtonEnum.CharCreate_Confirm To ButtonEnum.CharCreate_Close
            If CanShowButton(i) Then
                ...
                '//Render Button Text
                'Select Case i
                '    Case ButtonEnum.CharCreate_Confirm: ButtonText = "Confirm": DrawText = True
                '    Case Else: DrawText = False
                'End Select
                'If DrawText Then
                '    Select Case Button(i).State
                '        Case ButtonState.StateNormal: RenderText Font_Default, ButtonText, (.X + Button(i).X) + ((Button(i).Width / 2) - (GetTextWidth(Font_Default, ButtonText) / 2)) - 2, (.Y + Button(i).Y) + ((Button(i).Height / 2) - (8)) - 3, D3DColorARGB(255, 229, 229, 229), False
                '        Case ButtonState.StateHover: RenderText Font_Default, ButtonText, (.X + Button(i).X) + ((Button(i).Width / 2) - (GetTextWidth(Font_Default, ButtonText) / 2)) - 2, (.Y + Button(i).Y) + ((Button(i).Height / 2) - (8)) - 3, D3DColorARGB(255, 255, 255, 255), False
                '        Case ButtonState.StateClick: RenderText Font_Default, ButtonText, (.X + Button(i).X) + ((Button(i).Width / 2) - (GetTextWidth(Font_Default, ButtonText) / 2)) - 2, (.Y + Button(i).Y) + ((Button(i).Height / 2) - (8)) - 3 + 4, D3DColorARGB(255, 255, 255, 255), False
                '    End Select
                'End If
            End If
        Next
        ...
        '//Textbox
        RenderText Font_Default, UpdateChatText(Font_Default, CharName, 180) & TextLine, .X + 88, .Y + 46, White
        ...
        'RenderText Font_Default, "Character Creation", .X + 10, .Y + 5, D3DColorARGB(180, 255, 255, 255), False
        
        'RenderText Font_Default, "Name", .X + 22 + ((63 / 2) - (GetTextWidth(Font_Default, "Name") / 2)) - 2, .Y + 44, D3DColorARGB(200, 255, 255, 255), False
    End With
End Sub
    ...
    With GUI(GuiEnum.GUI_OPTION)
        ...
        For i = ButtonEnum.Option_Close To ButtonEnum.Option_sSoundDown
            ...
            If CanShowButton(i) Then
                ...
                '//Render Button Text
                'Select Case i
                '    Case ButtonEnum.Option_Control: ButtonText = "Control": DrawText = True
                '    Case Else: DrawText = False
                'End Select
                'If DrawText Then
                '    Select Case Button(i).State
        ...
        Select Case setWindow
            Case ButtonEnum.Option_Video
                '//Title
                'RenderText Font_Default, "Video Settings", .X + 110, .Y + 45, D3DColorARGB(255, 229, 229, 229), False
                
                '//Fullscreen
                tmpX = 105: tmpY = 45
                RenderText Font_Default, "Fullscreen", .X + tmpX + 25, .Y + tmpY, D3DColorARGB(255, 229, 229, 229), False
                If CursorX >= .X + tmpX And CursorX <= .X + tmpX + 17 And CursorY >= .Y + tmpY And CursorY <= .Y + tmpY + 17 And GuiZOrder(GuiVisibleCount) = GuiEnum.GUI_OPTION Then
                    RenderTexture Tex_Gui(.Pic), .X + tmpX, .Y + tmpY, 52, 403 + 17, 17, 17, 17, 17
                Else
                    RenderTexture Tex_Gui(.Pic), .X + tmpX, .Y + tmpY, 52, 403, 17, 17, 17, 17
                End If
                If isFullscreen = YES Then RenderTexture Tex_Gui(.Pic), .X + tmpX, .Y + tmpY, 52, 403 + 34, 17, 17, 17, 17
            Case ButtonEnum.Option_Sound
                tmpX = 105: tmpY = 45
                RenderText Font_Default, "Background", .X + tmpX, .Y + tmpY, D3DColorARGB(255, 229, 229, 229), False
                RenderText Font_Default, "Sound Effect", .X + tmpX, .Y + 27 + tmpY, D3DColorARGB(255, 229, 229, 229), False
                For i = 1 To MAX_VOLUME
                    If BGVolume >= i Then
                        RenderTexture Tex_System(gSystemEnum.UserInterface), .X + tmpX + 125 + ((8 + 3) * (i - 1)), .Y + tmpY, 0, 8, 9, 20, 1, 1, D3DColorARGB(255, 94, 177, 94)
                    Else
                        RenderTexture Tex_System(gSystemEnum.UserInterface), .X + tmpX + 125 + ((8 + 3) * (i - 1)), .Y + tmpY, 0, 8, 9, 20, 1, 1, D3DColorARGB(255, 100, 100, 100)
                    End If
                    If SEVolume >= i Then
                        RenderTexture Tex_System(gSystemEnum.UserInterface), .X + tmpX + 125 + ((8 + 3) * (i - 1)), .Y + 27 + tmpY, 0, 8, 9, 20, 1, 1, D3DColorARGB(255, 94, 177, 94)
                    Else
                        RenderTexture Tex_System(gSystemEnum.UserInterface), .X + tmpX + 125 + ((8 + 3) * (i - 1)), .Y + 27 + tmpY, 0, 8, 9, 20, 1, 1, D3DColorARGB(255, 100, 100, 100)
                    End If
                Next
            Case ButtonEnum.Option_Game
                '//Gui Path
                RenderText Font_Default, "GUI Path", .X + 105, .Y + 45, D3DColorARGB(255, 229, 229, 229), False
                RenderTexture Tex_System(gSystemEnum.UserInterface), .X + 165, .Y + 45, 0, 8, 180, 18, 1, 1, D3DColorARGB(100, 0, 0, 0)
            
                If GuiPathEdit Then
                    RenderText Font_Default, GuiPath & TextLine, .X + 167, .Y + 46, D3DColorARGB(255, 229, 229, 229), False
                Else
                    RenderText Font_Default, "...\" & GuiPath & "\", .X + 167, .Y + 46, D3DColorARGB(255, 229, 229, 229), False
                End If
                
                '//Fps
                tmpX = 105: tmpY = 70
                RenderText Font_Default, "Show FPS", .X + tmpX + 25, .Y + tmpY, D3DColorARGB(255, 229, 229, 229), False
                If CursorX >= .X + tmpX And CursorX <= .X + tmpX + 17 And CursorY >= .Y + tmpY And CursorY <= .Y + tmpY + 17 And GuiZOrder(GuiVisibleCount) = GuiEnum.GUI_OPTION Then
                    RenderTexture Tex_Gui(.Pic), .X + tmpX, .Y + tmpY, 52, 403 + 17, 17, 17, 17, 17
                Else
                    RenderTexture Tex_Gui(.Pic), .X + tmpX, .Y + tmpY, 52, 403, 17, 17, 17, 17
                End If
                If FPSvisible = YES Then RenderTexture Tex_Gui(.Pic), .X + tmpX, .Y + tmpY, 52, 403 + 34, 17, 17, 17, 17
            
                '//Ping
                tmpX = 105: tmpY = 90
                RenderText Font_Default, "Show Ping", .X + tmpX + 25, .Y + tmpY, D3DColorARGB(255, 229, 229, 229), False
                If CursorX >= .X + tmpX And CursorX <= .X + tmpX + 17 And CursorY >= .Y + tmpY And CursorY <= .Y + tmpY + 17 And GuiZOrder(GuiVisibleCount) = GuiEnum.GUI_OPTION Then
                    RenderTexture Tex_Gui(.Pic), .X + tmpX, .Y + tmpY, 52, 403 + 17, 17, 17, 17, 17
                Else
                    RenderTexture Tex_Gui(.Pic), .X + tmpX, .Y + tmpY, 52, 403, 17, 17, 17, 17
                End If
                If PingVisible = YES Then RenderTexture Tex_Gui(.Pic), .X + tmpX, .Y + tmpY, 52, 403 + 34, 17, 17, 17, 17
                
                '//Skip Boot Up
                tmpX = 105: tmpY = 110
                RenderText Font_Default, "Fast Boot Up", .X + tmpX + 25, .Y + tmpY, D3DColorARGB(255, 229, 229, 229), False
                If CursorX >= .X + tmpX And CursorX <= .X + tmpX + 17 And CursorY >= .Y + tmpY And CursorY <= .Y + tmpY + 17 And GuiZOrder(GuiVisibleCount) = GuiEnum.GUI_OPTION Then
                    RenderTexture Tex_Gui(.Pic), .X + tmpX, .Y + tmpY, 52, 403 + 17, 17, 17, 17, 17
                Else
                    RenderTexture Tex_Gui(.Pic), .X + tmpX, .Y + tmpY, 52, 403, 17, 17, 17, 17
                End If
                If tSkipBootUp = YES Then RenderTexture Tex_Gui(.Pic), .X + tmpX, .Y + tmpY, 52, 403 + 34, 17, 17, 17, 17
                
                '//Show Name
                tmpX = 105: tmpY = 130
                RenderText Font_Default, "Show Name", .X + tmpX + 25, .Y + tmpY, D3DColorARGB(255, 229, 229, 229), False
                If CursorX >= .X + tmpX And CursorX <= .X + tmpX + 17 And CursorY >= .Y + tmpY And CursorY <= .Y + tmpY + 17 And GuiZOrder(GuiVisibleCount) = GuiEnum.GUI_OPTION Then
                    RenderTexture Tex_Gui(.Pic), .X + tmpX, .Y + tmpY, 52, 403 + 17, 17, 17, 17, 17
                Else
                    RenderTexture Tex_Gui(.Pic), .X + tmpX, .Y + tmpY, 52, 403, 17, 17, 17, 17
                End If
                If Namevisible = YES Then RenderTexture Tex_Gui(.Pic), .X + tmpX, .Y + tmpY, 52, 403 + 34, 17, 17, 17, 17
                
                '//Language
                tmpX = 105: tmpY = 160
                RenderText Font_Default, "Language", .X + tmpX, .Y + tmpY, D3DColorARGB(255, 229, 229, 229), False
                For i = 1 To MAX_LANGUAGE
                    If (tmpCurLanguage + 1) = i Then
                        RenderTexture Tex_Misc(Misc_Language), .X + tmpX + 80 + ((i - 1) * 55), .Y + tmpY, 0, 25 * (i - 1), 45, 25, 45, 25, D3DColorARGB(255, 255, 255, 255)
                    Else
                        RenderTexture Tex_Misc(Misc_Language), .X + tmpX + 80 + ((i - 1) * 55), .Y + tmpY, 0, 25 * (i - 1), 45, 25, 45, 25, D3DColorARGB(100, 255, 255, 255)
                    End If
                Next
            Case ButtonEnum.Option_Control
                For i = 1 To MAX_CONTROL_PREV
                    ...
                    If Count > 0 And Count <= ControlEnum.Control_Count - 1 Then
                        RenderTexture Tex_Gui(.Pic), .X + 105, .Y + 43 + ((25 + 5) * (i - 1)), 93, 391, 229, 25, 229, 25
                    
                        '//Render Name
                        RenderText Font_Default, ControlKey(Count).keyName, .X + 105 + 4, .Y + 43 + ((25 + 5) * (i - 1)) + 3, White
                        
                        If editKey = Count Then
                            RenderTexture Tex_Gui(.Pic), .X + 212, .Y + 44 + ((25 + 5) * (i - 1)), 200, 417, 122, 24, 122, 24
                        End If
                        
                        RenderText Font_Default, GetKeyCodeName(TmpKey(Count)), .X + 212 + 4, .Y + 43 + ((25 + 5) * (i - 1)) + 3, White
                    End If
                Next
        End Select
        
        'RenderText Font_Default, "Settings", .X + 10, .Y + 5, D3DColorARGB(180, 255, 255, 255), False
    End With
End Sub
    ...
    With GUI(GuiEnum.GUI_CHATBOX)
        ...
        For i = ButtonEnum.Chatbox_Send To ButtonEnum.Chatbox_ScrollDown
            If CanShowButton(i) Then
                ...
                '//Render Button Text
                'Select Case i
                '    Case ButtonEnum.Chatbox_Send: ButtonText = "Send": DrawText = True
                '    Case Else: DrawText = False
                'End Select
                'If DrawText Then
                '    Select Case Button(i).State
        ...
        '//textbox
        If ChatOn Then
            RenderText Font_Default, UpdateChatText(Font_Default, MyChat & TextLine, 253), .X + 59, .Y + 144, White
        End If
        If EditTab Then
            RenderText Font_Default, UpdateChatText(Font_Default, ChatTab & TextLine, 38), .X + 6, .Y + 144, White
        Else
            RenderText Font_Default, PreviewChatText(Font_Default, ChatTab, 35), .X + 6, .Y + 144, White
        End If
...
Dim AddY As Long
    ...
    '//Server Time
    RenderText Ui_Default, KeepTwoDigit(GameHour) & ":" & KeepTwoDigit(GameMinute), Screen_Width - 161 - 5 + 5, 16 + 8, White
...
Dim RankIndex As Long
    ...
    With GUI(GuiEnum.GUI_RANK)
        ...
        '//ShowRank
        For i = 1 To 8
            RankIndex = (8 * RankScroll) + i
            If RankIndex > 0 And RankIndex <= 100 Then
                RenderTexture Tex_Gui(.Pic), .X + 30, .Y + 41 + (31 * (i - 1)), 28, 328, 212, 28, 212, 28
                ' Draw Index
                RenderText Font_Default, RankIndex & ": " & Trim$(Rank(RankIndex).Name) & " Lv" & Rank(RankIndex).Level, .X + 35, .Y + 45 + (31 * (i - 1)), White
            End If
        Next
    End With
End Sub
...
Dim SizeY As Long
    ...
    If InvItemDescTimer + 1200 > GetTickCount Then Exit Sub
